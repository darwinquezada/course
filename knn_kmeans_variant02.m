function [ results , clusters ] = knn_kmeans_variant02( database_orig , k, datarep, defaultNonDetectedValue, newNonDetectedValue , distanceMetric , k2 , randseed, labelk2,  folderKMeansBase, rho, labelrho)
...
for i = 1:csamples
	...
	if sum(clusters.idx == i) > thresholdClusterSize
		...
		for j = 1:nmacs
			idxsLarge{j,i} = vecidxsamples(((clusters.idx==i) .* database.trainingValidMacs(:,j))>0);
		end
