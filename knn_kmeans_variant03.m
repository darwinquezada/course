function [ results , clusters ] = knn_kmeans_variant03( database_orig , k, datarep, defaultNonDetectedValue, newNonDetectedValue , distanceMetric , k2 , randseed, labelk2,  folderKMeansBase, rho, labelrho)
...
%% Variant02 A Cluster is Large if it contain more than 4 times the expected number of samples in a equally distributed clustering 
specialCluster       = zeros(1,csamples);
thresholdClusterSize = 4 * (rsamples/k2);
idxsLarge            = cell(nmacs,csamples);
for i = 1:csamples
	
	if sum(clusters.idx == i) > thresholdClusterSize
		specialCluster(i) = 1;
	
		for j = 1:nmacs
			idxsLarge{j,i} = vecidxsamples(((clusters.idx==i) .* mask4Strongest(:,j) )>0); % in Variant03 they meet also the requirement of the strongest
		end
	
	end
	
end
% End of Variant02 precalculations
...
for i = 1:osamples  
	...
	% Variant03 Optimization
	if specialCluster(cluster)
		samples = idxsLarge{relevantAP,cluster};
	else
		samples = idxs{cluster};
	end
	% End of Variant03 Optimization	
