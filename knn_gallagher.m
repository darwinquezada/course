function [ results ] = knn_gallagher( database_orig , k, datarep, defaultNonDetectedValue, newNonDetectedValue , distanceMetric )
% knn_ips Indoor Positioning System based on kNN
% using the optimization rule based on Gallagher
...
% RSSI
%
% T. J. Gallagher, B. Li, A. G. Dempster, et al., 
% “A sector-based campus- wide indoor positioning 
% system,” in 2010 International Conference on 
...
validMacs = vecidxmacs(sum(database.trainingValidMacs)>0);
...
for i = 1:osamples  
    ...
    ofp = database.testMacs(i,validMacs);
	
	%% Gallagher optimized
	[value,validMac0] = max(ofp);
	validMac          = validMacs(validMac0);	
	samples = vecidxsamples(database.trainingValidMacs(:,validMac));
